...
const BidRequests = () => {
  ...
  const handelSubmit = async (id, prevStatus, status) => {
    if (prevStatus !== "In Progress") return console.log("not allowed");

    try {
      const { data } = await axios.patch(
        `http://localhost:9000/bid-status/${id}`,
        {
          status,
        }
      );
                    ...
                    <tr key={job._id}>
                      ...
                      <td className="px-4 py-4 text-sm whitespace-nowrap">
                        <div className="flex items-center gap-x-6">
                          <button
                            onClick={() =>
                              handelSubmit(job._id, job.status, "in progress")
                            }
                          ...
                          <button
                            onClick={() =>
                              handelSubmit(job._id, job.status, "rejected")
                            }
